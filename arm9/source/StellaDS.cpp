...
void LoadFavorites(void)
{
    ...
    FILE *fp = fopen("/data/StellaDS.fav", "rb");
    if (fp)
    {
...
void SaveFavorites(void)
{
    ...
    FILE *fp = fopen("/data/StellaDS.fav", "wb");
    if (fp)
    {
...
void dsDisplayFiles(unsigned int NoDebGame,u32 ucSel)
{
  ...
  sprintf(szName,"%s %s","A=RUN Y=HALT B=BACK SELECT=FAV");
  dsPrintValue(16-strlen(szName)/2,23,0,szName);
  for (ucBcl=0;ucBcl<17; ucBcl++) {
    ...
    if (ucGame < countvcs)
    {
      ...
      else
      {
        ...
        if (IsFavorite(vcsromlist[ucGame].filename))
        {
            dsPrintValue(0,5+ucBcl,(IsFavorite(vcsromlist[ucGame].filename) == 1) ? 1:0,(char*)"\\");
        }
        else
        {
...
unsigned int dsWaitForRom(void)
{
  ...
  while (!bDone)
  {
    ...
    if (keysCurrent() & KEY_SELECT)
    {
        ...
        {
            ToggleFavorite(vcsromlist[ucFicAct].filename);
            dsDisplayFiles(firstRomDisplay,romSelected);
            SaveFavorites();
