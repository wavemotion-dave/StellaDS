...
{
  gSystemCycles++;
  myDataBusState = cart_buffer[myCurrentOffset | (address & 0xFFF)];
  return myDataBusState;
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
ITCM_CODE void M6502Low::interruptHandlerF6()
{
  // Handle the interrupt
  if((myExecutionStatus & MaskableInterruptBit) && !I)
  {
    gSystemCycles += 7; // 7 cycle operation
    poke_F6(0x0100 + SP--, (PC - 1) >> 8);		// The high byte of the return address
    poke_F6(0x0100 + SP--, (PC - 1) & 0x00ff);	// The low byte of the return address
    poke_F6(0x0100 + SP--, PS() & (~0x10));	// The status byte from the processor status register
    D = false;	// Set our flags
    I = true;
    PC = (uInt16)peek_PCF6(0xFFFE) | ((uInt16)peek_PCF6(0xFFFF) << 8);	// Grab the address from the interrupt vector
  }
  else if(myExecutionStatus & NonmaskableInterruptBit)
  {
    gSystemCycles += 7; // 7 cycle operation
    poke_F6(0x0100 + SP--, (PC - 1) >> 8);
    poke_F6(0x0100 + SP--, (PC - 1) & 0x00ff);
    poke_F6(0x0100 + SP--, PS() & (~0x10));
    D = false;
    PC = (uInt16)peek_PCF6(0xFFFA) | ((uInt16)peek_PCF6(0xFFFB) << 8);
  }
...
void M6502Low::execute_F6(uInt16 number)
{
  ...
  for(;;)
  {
    ...
    for(; !myExecutionStatus && (fast_loop != 0); --fast_loop)
    {
      ...
      if (PC >= 0xFFF6) 
      {
          myDataBusState = peek_F6(PC++);
      }
      else 
      {
          gSystemCycles++;
          myDataBusState = cart_buffer[myCurrentOffset | (PC++ & 0xFFF)];
      }
