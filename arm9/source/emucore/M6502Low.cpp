...
ITCM_CODE uInt8 M6502Low::peek_DPCP(uInt16 address)
{
  ...
  if (address & 0x1000)
  {
      ...
      else if (address >= 0xFF4) 
      {
        switch (address)
        {
            case 0x0FF6:  myCurrentOffset = 0x0000;  return myDPC[myCurrentOffset | address];
            case 0x0FF7:  myCurrentOffset = 0x1000;  return myDPC[myCurrentOffset | address];
            case 0x0FF8:  myCurrentOffset = 0x2000;  return myDPC[myCurrentOffset | address];
            case 0x0FF9:  myCurrentOffset = 0x3000;  return myDPC[myCurrentOffset | address];
            case 0x0FFA:  myCurrentOffset = 0x4000;  return myDPC[myCurrentOffset | address];
            case 0x0FFB:  myCurrentOffset = 0x5000;  return myDPC[myCurrentOffset | address];
        }
      }
      return myDPC[myCurrentOffset | (address)];
  }
  else
  {
      if ((address & 0x280) == 0x80) return myRAM[address & 0x7F];
...
{
  gSystemCycles++;
  return myDPC[myCurrentOffset | (address & 0xFFF)];
}
...
void M6502Low::execute_DPCP(uInt16 number)
{
    ...
    while (fast_loop-- && !myExecutionStatus)
    {
      // Get the next 6502 instruction - do this the fast way!
      ++gSystemCycles;
      operand = myDPC[myCurrentOffset | (PC & 0xFFF)];
      PC++; 
